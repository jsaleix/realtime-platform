...
import { SOCKET_BOT_URL } from "../../constants/urls";
...
const MessageItem = ({ message, isCurrent, answerAction }) => {
                ...
                return (
                    <div>
                        <input
                            ...
                            onKeyDown={(e) =>
                                e.key === "Enter" && answerPrompt(dateValue)
                            }
                        />
                        <button
                            className="btn blue"
                            onClick={() => answerPrompt(dateValue)}
                        >
                            Send
                        </button>
                    </div>
                ...
                return (
                    <div>
                        <input
                            ...
                            onKeyDown={(e) =>
                                e.key === "Enter" && answerPrompt(stringValue)
                            }
                        />
                        <button
                            className="btn blue"
                            onClick={() => answerPrompt(stringValue)}
                        >
                            Send
                        </button>
                    </div>
                ...
                return (
                    <div>
                        <input
                            ...
                            onKeyDown={(e) =>
                                e.key === "Enter" && answerPrompt(intValue)
                            }
                        />
                        <button
                            className="btn blue"
                            onClick={() => answerPrompt(intValue)}
                        >
                            Send
                        </button>
                    </div>
    ...
    return (
        <div className={style.message}>
            ...
            {isCurrent && !answered && (
                <div className={style.actions}>{renderPrompt()}</div>
            )}
            {answered && (
...
const ChatBot = ({ close }) => {
    ...
    useEffect(() => {
        const tmpSocket = io(SOCKET_BOT_URL);
